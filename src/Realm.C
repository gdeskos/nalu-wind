...
Realm::look_ahead_and_creation(const YAML::Node& node)
{
  ...
  if (foundActuator.size() > 0) {
    ...
    // TODO delete the rest of this switch statement when remove old actuator
    // code
    const std::string actuatorTypeName =
      (*foundActuator[0])["actuator"]["type"].as<std::string>();
...
Realm::load(const YAML::Node& node)
{
  ...
  if (realmUsesEdges_) {
    ...
    NaluEnv::self().naluOutputP0()
      << "Element-based scheme will be activated" << std::endl;
  }
  ...
  if (!outputInfo_->catalystFileName_.empty()) {
    int error = Iovs::DatabaseIO::parseCatalystFile(
...
Realm::register_nodal_fields(stk::mesh::Part* part)
{
  ...
  if (realmUsesEdges_) {
    auto& edgeAreaVec = metaData_->declare_field<VectorFieldType>(
      stk::topology::NODE_RANK, "edge_area_vector");
    stk::mesh::put_field_on_mesh(
